...
[1]+d*u[1],o[2]+d*u[2]};if((p[0]<2)&(d>0)){A=l[0]*(p[0]-
  ...
  *(p[2]-c[2]);delta=A*A-(B-r*r);I=0.4;if(delta>0){
    I*=(1-sqrt(delta)/(2*r))*(1-sqrt(delta)/(2*r));}}else{I=0;}}S=asc[(int)
      (I*asclen)];return S;}
        void render(){char c[3241];c[3240]='\0';for(int k=0;k<
         100000;++k){for(int i=0;i<40;++i){for(int j=0;j<40;++j){c[81*i
            +2*j]=shade(i,j,(float)k);c[81*i+2*j+1]=c[81*i
                 +2*j];}c[81*i+80]='\n';}printf("\033[2J\033[1;1H");
                     printf("%s\n",c);sleep(0.005);}}int 
                            main(){render();return 0;}
